...
import com.github.klboke.nexusplus.persistence.mysql.dao.DockerAuthTokenDao;
...
class DockerUploadCleanupWorker {
  ...
  DockerUploadCleanupWorker(
      ...
      @Value("${nexus-plus.docker.cleanup.enabled:true}") boolean enabled,
      @Value("${nexus-plus.docker.cleanup.batch-size:64}") int batchSize,
      @Value("${nexus-plus.docker.cleanup.lease-seconds:300}") long leaseSeconds) {
  ...
  @Scheduled(
      fixedDelayString = "${nexus-plus.docker.cleanup.interval-ms:60000}",
      initialDelayString = "${nexus-plus.docker.cleanup.initial-delay-ms:60000}")
  public void cleanup() {
    if (!enabled) {
      return;
    }
